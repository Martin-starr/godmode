...
// 9s abort: fails cleanly inside guarded()'s 12s watchdog instead of
// tripping it (a tripped watchdog resets the DB pool for nothing).
export async function fetchGmailThreads() {
  const { url, token } = bridgeConfig();
  const res = await fetch(url + "?action=gmail&token=" + encodeURIComponent(token), {
    redirect: "follow",
