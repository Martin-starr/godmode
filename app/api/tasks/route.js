...
export const POST = guarded(
  async (req) => {
    ...
    const sql = db();
    const rows = await sql`insert into dash.tasks (title, sub, tag, tagcls, who, open)
      values (${title}, ${String(body.sub || "")}, ${String(body.tag || "Ny")}, ${tagcls(body.tag)}, ${String(body.who || "")}, 1)
      returning id, title, sub, tag, tagcls, who, open`;
    return json(rows[0]);
  },
  { edit: true }
);
